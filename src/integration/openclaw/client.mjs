...
// --- Device Auth Payload (ported from openclaw/src/gateway/device-auth.ts) ---

function buildDeviceAuthPayload({ deviceId, clientId, clientMode, role, scopes, signedAtMs, token, nonce }) {
  const version = nonce ? 'v2' : 'v1';
  const scopeStr = scopes.join(',');
  const tokenStr = token || '';
  const base = [version, deviceId, clientId, clientMode, role, scopeStr, String(signedAtMs), tokenStr];
  if (version === 'v2') {
    base.push(nonce || '');
  }
  return base.join('|');
...
// --- Device Auth Token Store (ported from openclaw/src/infra/device-auth-store.ts) ---
...
/**
 * OpenClaw Client for connecting to OpenClaw Gateway.
 * Implements the WebSocket protocol: connect.challenge -> connect -> hello-ok
 * Includes device identity authentication matching the openclaw gateway protocol.
 */
      ...
      this.ws.on('message', (data) => {
        try {
          ...
          if (msg.type === 'event' && msg.event === 'connect.challenge') {
            ...
            // Build device auth
            const storedToken = loadDeviceAuthToken(this.deviceIdentity.deviceId, ROLE)?.token;
            const authToken = storedToken || this.authToken || undefined;
            ...
            this.pendingRequests.set(requestId, {
              resolve: (helloOk) => {
                ...
                // Store device auth token if returned
